...
public class Infointraction : MonoBehaviour
{
    ...
    public AudioClip newSystemAudioClip;
    ...
    public void enableAll(AudioSource thisAudio = null)
    {
        if (useNewSystem)
        {
            Debug.Log("Using new interaction system");
            GameDataManager.Instance.FillChecklist(this.tag, newSystemAudioClip);
        }
        else
        {//this is all the old stuff
            Debug.Log("Using OLD interaction system");
            if (thisAudio == null)
